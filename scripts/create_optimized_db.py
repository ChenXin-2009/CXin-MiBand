...
OUTPUT_DB = os.path.join(os.path.dirname(__file__), '..', 'temp', 'optimized_dict.db')
...
for row in source_cursor.fetchall():
    ...
    try:
        output_cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?)
        ''', (word, phonetic or '', translation, collins or 0, oxford or 0))
...
# 牛津词汇
cursor.execute('SELECT COUNT(*) FROM dict WHERE oxford > 0')
oxford_count = cursor.fetchone()[0]
print(f'牛津3000词汇: {oxford_count:,}')

# 柯林斯词汇
for level in [5, 4, 3, 2, 1]:
    cursor.execute('SELECT COUNT(*) FROM dict WHERE collins = ?', (level,))
    count = cursor.fetchone()[0]
    print(f'柯林斯{level}星: {count:,}')
...
# 采样显示
print('\n=== 随机采样 (10个词) ===\n')
cursor.execute('SELECT word, phonetic, translation, collins, oxford FROM dict ORDER BY RANDOM() LIMIT 10')
...
print('数据库创建完成！')
print(f'\n运行 "python scripts/query_dict.py" 来测试查询功能')
